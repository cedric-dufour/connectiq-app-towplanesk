...
class TSK_Altimeter {
  ...
  function reset() {
    ...
    self.fQFE = null;
    // ... filter
    $.TSK_oFilter.resetFilter(TSK_Filter.PRESSURE);
    ...
    // ... filter
    $.TSK_oFilter.resetFilter(TSK_Filter.TEMPERATURE);
  }

  function importSettings() {
    // QNH
    self.fQNH = $.TSK_oSettings.fAltimeterCalibrationQNH;
  }
  ...
  function setQFE(_fQFE) {  // [Pa]
    ...
    // Calibrated value
    var fValue = $.TSK_oFilter.filterValue(TSK_Filter.PRESSURE, self.fQFE_raw * $.TSK_oSettings.fAltimeterCorrectionRelative + $.TSK_oSettings.fAltimeterCorrectionAbsolute, true);
    if(fValue == null) {
      return;
    }
    self.fQFE = fValue;
  ...
  function setTemperatureActual(_fTemperatureActual) {  // [°K]
    ...
    if(_fTemperatureActual != null) {
      self.fTemperatureActual = $.TSK_oFilter.filterValue(TSK_Filter.TEMPERATURE, _fTemperatureActual, false);  // no strict filtering; assume temperature sensor isn't too fishy
  ...
  function postProcess() {
    ...
    if(!self.bTemperatureActualSet or self.fTemperatureActual == null) {
      self.fTemperatureActual = self.fTemperatureISA + $.TSK_oSettings.fTemperatureISAOffset;
    }
