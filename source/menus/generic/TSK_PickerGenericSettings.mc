...
class TSK_PickerGenericSettings extends Ui.Picker {
  ...
  function initialize(_context, _item) {
    if(_context == :contextTimer) {
      if(_item == :itemThresholdGround) {
        ...
        var oFactory = new PickerFactoryDictionary([5, 10, 15, 30, 45, 60, 90, 120, 180, 300], ["5", "10", "15", "30", "45", "60", "90", "120", "180", "300"], null);
        var iIndex = oFactory.indexOfKey(iTimerThresholdGround);
        if(iIndex < 0) {
          iIndex = 3;  // 30 seconds
        }
        Picker.initialize({
          :title => new Ui.Text({ :text => Lang.format("$1$ [s]", [Ui.loadResource(Rez.Strings.titleTimerThresholdGround)]), :font => Gfx.FONT_TINY, :locX=>Ui.LAYOUT_HALIGN_CENTER, :locY=>Ui.LAYOUT_VALIGN_BOTTOM, :color => Gfx.COLOR_BLUE }),
          :pattern => [ oFactory ],
          :defaults => [ iIndex ]
        });
      }
        ...
        Picker.initialize({
          :title => new Ui.Text({ :text => Lang.format("$1$ [s]", [Ui.loadResource(Rez.Strings.titleTimerThresholdAirborne)]), :font => Gfx.FONT_TINY, :locX=>Ui.LAYOUT_HALIGN_CENTER, :locY=>Ui.LAYOUT_VALIGN_BOTTOM, :color => Gfx.COLOR_BLUE }),
          :pattern => [ oFactory ],
          :defaults => [ iIndex ]
        });
      }
      ...
      if(_item == :itemDistance) {
        ...
        var oFactory = new PickerFactoryDictionary([-1, 0, 1], [Ui.loadResource(Rez.Strings.valueAuto), "kg", "lb"], null);
        Picker.initialize({
          :title => new Ui.Text({ :text => Ui.loadResource(Rez.Strings.titleUnitWeight), :font => Gfx.FONT_TINY, :locX=>Ui.LAYOUT_HALIGN_CENTER, :locY=>Ui.LAYOUT_VALIGN_BOTTOM, :color => Gfx.COLOR_BLUE }),
          :pattern => [ oFactory ],
        ...
        var oFactory = new PickerFactoryDictionary([-1, 0, 1 ,2], [Ui.loadResource(Rez.Strings.valueAuto), "l", "gal", "kWh"], null);
        Picker.initialize({
          :title => new Ui.Text({ :text => Ui.loadResource(Rez.Strings.titleUnitFuel), :font => Gfx.FONT_TINY, :locX=>Ui.LAYOUT_HALIGN_CENTER, :locY=>Ui.LAYOUT_VALIGN_BOTTOM, :color => Gfx.COLOR_BLUE }),
          :pattern => [ oFactory ],
        ...
        var oFactory = new PickerFactoryDictionary([-1, 0, 1], [Ui.loadResource(Rez.Strings.valueAuto), "°C", "°F"], null);
        Picker.initialize({
...
class TSK_PickerGenericSettingsDelegate extends Ui.PickerDelegate {
  ...
  function onAccept(_amValues) {
    if(self.context == :contextTimer) {
      ...
      $.TSK_oSettings.load();  // ... use proper units in settings
    }
