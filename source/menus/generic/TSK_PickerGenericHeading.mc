...
class TSK_PickerGenericHeading extends PickerGenericHeading {
  ...
  function initialize(_context, _item) {
    if(_context == :contextSettings) {
      if(_item == :itemWindDirection) {
        PickerGenericHeading.initialize(Ui.loadResource(Rez.Strings.titleWindDirection), App.Properties.getValue("userWindDirection"), 0, false);
      }
    }
  }
...
class TSK_PickerGenericHeadingDelegate extends Ui.PickerDelegate {
