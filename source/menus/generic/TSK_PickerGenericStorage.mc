...
class TSK_PickerGenericStorage extends Ui.Picker {
  ...
  function initialize(_storage, _action) {
    ...
    var aiStorageKeys = new [$.TSK_STORAGE_SLOTS];
    var asStorageValues = new [$.TSK_STORAGE_SLOTS];
    var iStorageUsed = 0;
    // ... storage specifics
...
class TSK_PickerGenericStorageDelegate extends Ui.PickerDelegate {
    ...
    if(_amValues[0] != null) {
      ...
      if(self.storage == :storageTowplane) {
        ...
        if(self.action == :actionSave) {
          var o = $.TSK_oTowplane != null ? $.TSK_oTowplane : new TSK_Towplane({});
          ...
          $.TSK_oTowplane.import(App.Storage.getValue(Lang.format("stor$1$$2$", [sStorageName, s])));
        }
        ...
        sStorageName = "Glider";
        if(self.action == :actionSave) {
          var o = $.TSK_oGlider != null ? $.TSK_oGlider : new TSK_Glider({});
          App.Storage.setValue(Lang.format("stor$1$$2$", [sStorageName, s]), o.export());
        }
        else if(self.action == :actionLoad) {
          if($.TSK_oGlider == null) {
          ...
          $.TSK_oGlider.import(App.Storage.getValue(Lang.format("stor$1$$2$", [sStorageName, s])));
        }
        else if(self.action == :actionDelete) {
          App.Storage.deleteValue(Lang.format("stor$1$$2$", [sStorageName, s]));
        }
