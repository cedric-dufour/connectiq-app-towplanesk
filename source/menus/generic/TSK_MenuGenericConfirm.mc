...
class TSK_MenuGenericConfirm extends Ui.Menu {
...
class TSK_MenuGenericConfirmDelegate extends Ui.MenuInputDelegate {
  ...
  function onMenuItem(item) {
    if(self.context == :contextActivity) {
      if(self.action == :actionStart) {
        ...
        if($.TSK_oActivity != null) {
          $.TSK_oActivity.stop(true);
          $.TSK_oActivity = null;
        }
      }
      else if(self.action == :actionDiscard) {
        if($.TSK_oActivity != null) {
          $.TSK_oActivity.stop(false);
          $.TSK_oActivity = null;
        }
      }
    }
