...
class PickerGenericFuelFlow extends Ui.Picker {
  ...
  function initialize(_sTitle, _fValue, _iUnit, _bAllowNegative) {
    ...
    if(_iUnit == 2) {
      ...
      sUnit = "gal/h";
      iMaxSignificant = 3;
      _fValue *= 9510193.88489f;  // m3/s -> gal/h (* 10)
      if(_fValue > 3999.0f) {
        _fValue = 3999.0f;
      }
      else if(_fValue < -3999.0f) {
        _fValue = -3999.0f;
      }
    }
    else {
      sUnit = "l/h";
      iMaxSignificant = 9;
      _fValue *= 36000000.0f;  // m3/s -> l/h (* 10)
      if(_fValue > 9999.0f) {
        _fValue = 9999.0f;
      }
      else if(_fValue < -9999.0f) {
  ...
  function getValue(_amValues, _iUnit) {
    ...
    // Use user-specified fuel unit (NB: SI units are always used internally)
    if(_iUnit == 2) {
      fValue /= 10000.0f;  // kW (* 10) -> J/s
    }
    else if(_iUnit == Sys.UNIT_STATUTE) {
      fValue /= 9510193.88489f;  // gal/h (* 10) -> m3/s
    }
    else {
      fValue /= 36000000.0f;  // l/h (* 10) -> m3/s
    }
