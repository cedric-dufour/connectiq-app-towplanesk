...
class PickerGenericFuelDensity extends Ui.Picker {
  ...
  function initialize(_sTitle, _fValue, _iUnit, _bAllowNegative) {
    ...
    // PRECISION: 0.001 (* 1000)
    var sUnit;
    var iMaxSignificant;
    if(_iUnit == Sys.UNIT_STATUTE) {
      sUnit = "lb/gal";
      iMaxSignificant = 19;
      _fValue *= 8.345404452f;  // kg/m3 -> lb/gal (* 1000)
      if(_fValue > 19999.0f) {
        _fValue = 19999.0f;
      }
      else if(_fValue < -19999.0f) {
        _fValue = -19999.0f;
      }
    }
    else {
  ...
  function getValue(_amValues, _iUnit) {
    ...
    // Use user-specified fuel unit (NB: SI units are always used internally)
    if(_iUnit == Sys.UNIT_STATUTE) {
      fValue /= 8.345404452f;  // lb/gal (* 1000) -> kg/m3
    }
    //else {
    //  fValue /= 1.0f;  // kg/l (* 1000) -> kg/m3
    //}
